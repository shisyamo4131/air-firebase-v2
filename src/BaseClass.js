...
export class BaseClass {
   ...
   * [NOTE]
   * Firestore の `onSnapshot` を利用して読み込んだドキュメントをインスタンスに設定している場合、
   * 当該ドキュメントに対して `runTransaction` を利用すると `保留中` 状態にドキュメントが更新され
   * 再読み込みが発生します。結果、`initialize` が実行されるために `_beforeData` の内容が
   * 更新されることに注意してください。
   * 1. インスタンスのプロパティを変更する。
   * 2. `runTransaction` を呼び出す。
   * 3. Firestore 上の当該ドキュメントの状態が `保留中` に更新される。
   * 4. `onSnapshot` リスナーが反応してインスタンスの `initialize` が実行される。
   * 5. `_beforeData` も更新される。
   * よって、当該メソッドはデータベース更新エラー発生時の復元ではなく、UI 上の
   * ユーザビリティとして利用することを推奨します。
   * なお、FireModel の CUD 処理はすべてトランザクション処理が内部で実装されているため、
   * サブクラス側で `runTransaction` を利用していなくても当該メソッドは期待通りの結果を得られません。
   * データベース更新エラー発生時の復元処理は `clone()` を利用して更新前のインスタンスを保持し、
   * エラー発生時に復元する方法を推奨します。
   *****************************************************************************/
  undo() {
